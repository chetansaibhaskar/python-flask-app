pipeline {
    ...
    environment {
        ...
        GIT_SCM = 'https://github.com/chetansaibhaskar/python-flask-app.git'
    }
    stages {
        stage ("Clean WS") {
            steps {
        ...
        stage('Deploy to Prod') {
            steps {
                withCredentials([sshUserPrivateKey(credentialsId: 'ssh-key', keyFileVariable: 'MY_SSH_KEY', usernameVariable: 'username')]) {
                    sh '''
                    scp -i $MY_SSH_KEY -o StrictHostKeyChecking=no /var/lib/jenkins/workspace/singleserverdep/python.zip  ${username}@${SERVER_IP}:/home/ec2-user/
